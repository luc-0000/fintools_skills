...
2. 先确认用户当前想跑哪一个股票代码
3. 如果没有设置 pool，则询问用户是直接按该股票运行，还是打开 UI 为这个 agent 配置 pool
4. 只有在用户确认后，才进入股票池配置流程
5. 如果用户不设置 pool，系统也不能自动代替用户创建任何临时 pool

如果用户选择 UI，说明需要保持简短，只需要覆盖：

- 去 `Pools` 新建 pool
- 给 pool 加股票
- 去 `Rules` 把 pool assign 给目标 agent
- 用 `Run Today` 跑整池，或用单股 `Run` 跑指定股票
